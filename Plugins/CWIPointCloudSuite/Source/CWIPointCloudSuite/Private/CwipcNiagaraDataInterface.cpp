...
void UCwipcNiagaraDataInterface::LockPointCloud(FVectorVMExternalFunctionContext& Context)
{
	...
	else
	{
		...
		if (warnedAboutLockBeforeInitialize)
		{
			UE_LOG(LogTemp, Warning, TEXT("UCwipcNiagaraDataInterface[%s]::LockPointCloud(): InitializeSource apparently called, source=[%s]"), *GetPathNameSafe(this), *GetPathNameSafe(CwipcPointCloudSourceAsset));
			warnedAboutLockBeforeInitialize = false;
		}
	}
